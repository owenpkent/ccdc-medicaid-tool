...
- **The engine (the write side) is ported into `web/src/lib` (2026-07-01/02).** From the proven CDASS Enroll modules: `lib/profile/schema.ts` (the archive schema; Medicaid sections still to come with CCDC), `lib/fill/util.ts` (tolerant pdf-lib helpers), `lib/fill/forms/` (the CDASS/PPL Attendant Packet 2026 with its embedded I-9), and `lib/extract/packet2026.ts` (carry-forward: reads the typed answers back out of a previously filled packet). The blank official template ships in `web/public/forms/` with a registry entry in `rules/co/forms/`; exact-copy and round-trip tests run against the real 22-page PDF. The `#fill` view (early preview, behind the consent gate) does edit, check-every-answer review, generate, then an on-screen review-and-sign step (view the filled packet and optionally add a signature) before the local download; the archive (`lib/archive.ts`) is the single audited storage module, opt-in only, with a privacy-guard test enforcing that nothing else ever touches storage. Capture is ported too (2026-07-02): license-barcode (zxing-wasm), license-front, passport-MRZ, and Social Security card scans fill the form on-device in `lib/extract`, and a fictional example person plus a committed scannable example ID make the flow demonstrable without real documents. Still ahead: the Medicaid schema sections, the first Medicaid form mapping, and Spanish field labels.
...
3. **Done (2026-07-01/02): the engine is ported into the shell.** Schema, tolerant fill helpers, the CDASS packet mapping with embedded I-9, carry-forward import, the `#fill` preview UI, the opt-in archive, and now the capture pipeline (license barcode, license front, passport MRZ, Social Security card, all decoded on-device) are all in `web/src/lib` and live in the demo, with a fictional example person and scannable example ID for demonstrations. The follow-on work is content: the Medicaid schema sections (with CCDC), the first Medicaid form mapping (needs the real form), and Spanish field labels.
